...
- `state/display`
  ...
  - `correctAnswer`: the text of the correct option for the current question, when the prepared question declares one. It lives here, in the admin-only document, rather than in the publicly readable `state/live`, so that participants cannot read it ahead of time.
  - `showAnswer`: whether the correct answer is boxed on the results page. Independent of `reveal`, so the votes stay visible next to it.
...
9. Revealing answers only changes their visibility. When the admin finishes a question, one batch clears its cached tallies, sets `state/live.status` to `complete`, and resets `state/display.reveal`, `showQuestion` and `showAnswer` to false. Reopening or reactivating also clears cached tallies.
...
- The correct answer, when a prepared question declares one, is boxed with a CSS `outline` on the label (`.sp-correct`). An outline is drawn outside the box, so revealing it moves nothing else on the screen. Its color comes from `correctColor` in the selected results style.
- Each answer label gets a box as wide as its column of bubbles, never wider than twice its distance to the nearest edge of the screen, so it cannot run off. Plain text wraps inside that box; a formula cannot wrap, so `schoen-poll.js` measures the typeset math and, when needed, shrinks all the labels by one common factor (down to 0.35). The measurement is redone once KaTeX and its fonts have loaded. Labels are laid out when a question is launched and are not recomputed on window resize.
